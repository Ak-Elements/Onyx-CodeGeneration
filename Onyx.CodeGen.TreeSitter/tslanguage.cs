...
namespace Onyx.CodeGen.TreeSitter
{
    public sealed class TSLanguage : IDisposable
    {
        ...
        public TSLanguage(IntPtr ptr)
        {
            ...
            {
                string? sym = Marshal.PtrToStringAnsi(ts_language_symbol_name(Ptr, i));
                symbols[i] = sym == null ? "" : sym;
            }
            ...
            {
                string? field = Marshal.PtrToStringAnsi(ts_language_field_name_for_id(Ptr, i));
                fields[i] = field == null ? "" : field ;
                if (string.IsNullOrEmpty(fields[i]) == false)
                {
